...
class GroqClient:
    ...
    def extract_score_from_result(self, result_raw):
        ...
        if match:
            ...
            print(f"Score capturado: {score_str}")  # Log para verificar o valor capturado
            if '/' in score_str:
            ...
            try:
                ...
                print(f"Erro ao converter '{score_str}' para float.")
                return None
        
        # Retornar None se não encontrar a pontuação
        print("Pontuação não encontrada no resultado:")
        print(result_raw)
        return None
